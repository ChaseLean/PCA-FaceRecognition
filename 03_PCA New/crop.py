...
img = cv2.imread(os.path.join(path, labels[0]))  # read image in BGR
...
for label in labels:
    try:
        ...
        # Step -1 Read Image and Convert to RGB
        img = cv2.imread(os.path.join(path, label))  # read image in BGR
        # this step will convert image from BGR to RGB
        img_rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        ...
        for x, y, w, h in faces_list:
            ...
            roi = img[y:y+h, x:x+w]
            # Step -4: Save Image
            cv2.imwrite(f'./train_images/{label}', roi)
